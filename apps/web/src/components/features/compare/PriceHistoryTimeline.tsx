...
export function PriceHistoryTimeline({ history }: { history: PriceHistoryItem[] }) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
      ...
      <div className="relative space-y-4 before:absolute before:inset-0 before:ml-[13px] before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-primary-200 before:via-border before:to-transparent">
        {history.map((item, index) => {
          ...
          return (
            <div key={item.id} className="relative flex items-start gap-4 pl-0">
              ...
              <div className={`mt-1.5 w-6 h-6 rounded-full border-2 border-primary-100 dark:border-white/5 flex items-center justify-center z-10 shrink-0 ${
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 flex justify-between items-start min-w-0">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-muted-foreground truncate">{item.date}</p>
                    </div>
